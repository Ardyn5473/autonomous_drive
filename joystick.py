...
class KeyboardController(object):
    ...
    def poll(self):
        ...
        if keys[pygame.K_w] or keys[pygame.K_UP]:
            ...
            self.throttle = config.REVERSE  # ブレーキ
...
try:
    ...
    class JoystickROSNode(Node):
        ...
        def publish_data(self):
            ...
            self.mode_pub.publish(String(data=self.joystick.mode[0]))
            ...
            self.get_logger().info(
                f"Published: steering={self.joystick.steering}, throttle={self.joystick.throttle}, "
                f"mode={self.joystick.mode[0]}, recording={self.joystick.recording}, "
                f"buttons={self.joystick.button_states}"
            )
