    ...
    def __init__(self, model, inference_engine=None):
        ...
        Args:
            ...
            inference_engine: 推論エンジン ("pytorch", "tensorrt", "openvino")
                             Noneの場合はconfig.INFERENCE_ENGINEを使用
        """
        self.model = model
        self.inference_engine = inference_engine or config.INFERENCE_ENGINE
...
def load_model_with_engine(model_path, model_type=None, inference_engine=None, **kwargs):
    ...
    Returns:
        model: 読み込まれたモデル
    """
    inference_engine = inference_engine or config.INFERENCE_ENGINE
...
def _load_tensorrt_model(model_path, model_type, **kwargs):
    ...
    try:
        ...
        if model_type == "yolo":
            ...
            # 自動運転モデル・位置推論モデルのTensorRT (torch2trt)
            # TensorRT形式のモデルパスを確認
            if model_path.endswith('.pth'):
                tensorrt_path = model_path.replace('.pth', '_tensorrt.pt')
            elif model_path.endswith('.pt'):
                tensorrt_path = model_path.replace('.pt', '_tensorrt.pt')
            else:
                tensorrt_path = model_path + '_tensorrt.pt'

            if not os.path.exists(tensorrt_path):
                logger.warning(f"TensorRT model not found: {tensorrt_path}")
                logger.info("Falling back to PyTorch model. To create TensorRT model, use torch2trt")
                return _load_pytorch_model(model_path, model_type, **kwargs)
            ...
            model_trt = TRTModule()
            model_trt.load_state_dict(torch.load(tensorrt_path, map_location=device))
            logger.info(f"Loaded TensorRT model (torch2trt): {tensorrt_path}")
            return model_trt
