...
try:
    ...
    class IMUNode(Node):
        def __init__(self):
            ...
            self.imu = BNO055()  # BNO055インスタンスを作成
        ...
        def euler_to_quaternion(self, roll, pitch, yaw):
                """
                オイラー角（roll, pitch, yaw）をクォータニオンに変換する。
                roll: x軸周りの回転
                pitch: y軸周りの回転
                yaw: z軸周りの回転
                """
        ...
        def publish_imu_data(self):
                try:
                    ...
                    # クォータニオンを取得
                    qx, qy, qz, qw = self.imu.getQuat()
                    
                    """# オイラー角からクォータニオンを計算する場合
                    roll = math.radians(imu_angle["x"])
                    pitch = math.radians(imu_angle["y"])
                    yaw = math.radians(imu_angle["z"])
                    ...
                    # 角速度データを設定
                    imu_msg.angular_velocity.x = imu_angular_velocity["x"][-1]
                    imu_msg.angular_velocity.y = imu_angular_velocity["y"][-1]
                    imu_msg.angular_velocity.z = imu_angular_velocity["z"][-1]
                    ...
                    # 角度データ（オイラー角）を設定
                    # ここでは、単純化のため角度をログで確認
                    self.get_logger().info(f"Position: x={imu_position['x']}, y={imu_position['y']}, z={imu_position['z']}")

                    # トピックにパブリッシュ
                    self.publisher.publish(imu_msg)
                    # TFをブロードキャスト
                    #self.broadcast_tf()
                    self.broadcast_tf(qx, qy, qz, qw, imu_position)
                    self.get_logger().info(f"Published IMU data")

                except Exception as e:
                    self.get_logger().error(f"Error while publishing IMU data: {e}")

        def broadcast_tf(self, qx, qy, qz, qw, position):
            # TFメッセージを作成
            t = TransformStamped()

            # ヘッダー情報
            t.header.stamp = self.get_clock().now().to_msg()
            t.header.frame_id = "base_link"  # 基準フレーム
            t.child_frame_id = "imu_frame"  # IMUのフレーム

            # TF変換 (ここでは原点に固定)
            t.transform.translation.x = 0.0 #position["x"]
            t.transform.translation.y = 0.0 #position["y"]
            t.transform.translation.z = 0.0 #position["z"]
