...
class LidarBase(ABC):
    ...
    def _convert_units(self, points):
        ...
        # LiDARデータ保存用
        self.save_lidar_binary = getattr(cfg, 'SAVE_LIDAR_DATA', True)
    ...
    # def format_detection_output(self, detection_details, detection_binary):
    #     """
    #     検出結果を統一フォーマットで出力 - 実際に検出された場合のみ表示
        
    #     Args:
    #         detection_details: 検出詳細のリスト
    #         detection_binary: バイナリ検出結果
    #     """
    #     # バイナリ検出結果で実際に検出があった場合のみ表示
    #     if detection_details and any(detection_binary):
    #         print(f"障害物検出: {detection_details} | detection={detection_binary}")
    ...
    def calculate_zone_detection_distances(self, points, zone_index, detect_points_threshold, max_distance, zone_thresholds=None, distance_thresholds=None):
        """
        各ゾーンの検知距離とバイナリベクトルを計算し、検出時に出力する統合処理
        ...
        Returns:
            tuple: (detection_distances, detection_binary, zone_all_distances)
        """
        detection_distances = []
        
        # configからゾーン名を取得
        try:
            zone_names = getattr(cfg, 'ZONE_NAMES', [f'Zone{i}' for i in range(len(zone_index))])
        except ImportError:
            zone_names = [f'Zone{i}' for i in range(len(zone_index))]
            ...
            if zone < len(zone_index) and zone_index[zone]:
                ...
                if np.any(valid_mask):
                    ...
                    if detection_count > 0:
                        median_distance = float(np.median(zone_distances[detection_mask]))
                    else:
                        median_distance = max_distance #0.0
                else:
                    detection_count = 0
                    median_distance = max_distance #0.0
            else:
        ...
        #print(detection_distances, detection_binary)
        return detection_distances, detection_binary
    ...
    def detect(self, points):
        ...
        """
        # 統合処理で検知距離とバイナリベクトルを計算（出力も含む）
        detection_distances, detection_binary = self.calculate_zone_detection_distances(
            points, self.zone_index, self.detect_points_threshold, self.max_distance, 
            self.zone_thresholds, self.distance_thresholds
        )
        
        # 各ゾーンの全点中央値を計算
        zone_distances = self.calculate_zone_median_distances(points)
    ...
    def calculate_zone_median_distances(self, points):
        ...
        if points is None or len(points) == 0:
            return [0.0] * len(self.zone_index)
...
def main():
    ...
    lidar = create_lidar(lidar_type=cfg.LIDAR_TYPE, ip_address='192.168.0.10', image_w=224, image_h=224)
