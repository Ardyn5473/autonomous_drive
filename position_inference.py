...
def load_position_model():
    ...
    try:
        ...
        if position_model is not None:
            logger.info(f"位置推論モデルをロードしました: {config.POSITION_MODEL_NAME} ({config.POSITION_MODEL_TYPE}, {config.INFERENCE_ENGINE})")
...
def infer_position(position_model, camera_image):
    ...
    try:
        ...
        # 統一的な推論インターフェースを使用
        inference_wrapper = ModelInference(position_model, config.INFERENCE_ENGINE)

        # 推論エンジンに応じて適切なデバイスに配置
        if config.INFERENCE_ENGINE == "pytorch" or config.INFERENCE_ENGINE == "tensorrt":
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
