...
if [ -z "$AVAILABLE_HEADERS" ]; then
    echo "⚠️ Jetson用linux-headersが見つかりません。nvidia-jetpack-devをインストールします..."
    sudo apt install -y dkms build-essential
    sudo apt install -y nvidia-jetpack-dev || {
        echo "⚠️ nvidia-jetpack-devのインストールに失敗。基本パッケージのみでビルドを試行します..."
        sudo apt install -y dkms build-essential
    }
else
...
# DKMSにモジュールを追加
echo "⚙️ DKMSにxpadモジュールを追加..."
sudo dkms add .

# モジュールをビルド
echo "🛠️ xpadモジュールをビルド..."
...
echo ""
echo "🔍 インストール確認:"
lsmod | grep xpad || echo "⚠️ xpadモジュールがロードされていません"
echo ""
echo "🎮 接続されているジョイスティック:"
ls /dev/input/js* 2>/dev/null || echo "⚠️ ジョイスティックデバイスが見つかりません"
echo ""
