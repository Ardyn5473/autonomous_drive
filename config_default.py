...
RECOVERY_BRAKING = 1             # ブレーキ回数（Reverseを利用）
...
# 推論エンジンの選択（すべてのモデルに適用）
# "pytorch":  通常のPyTorchモデル（学習時・開発時推奨）
# "tensorrt": TensorRT最適化モデル（Jetson推奨）
# "openvino": OpenVINO最適化モデル（x86/RPi推奨）
...
NUM_CATEGORIES = 3
# 小さな値→大きな値の順にする（bins must increase monotonically）
CATEGORIES_STEERING = [RIGHT, NEUTRAL, LEFT]
CATEGORIES_THROTTLE = [FORWARD_CORNER, FORWARD_STRAIGHT, FORWARD_CORNER]
...
# オプティカルフローセンサー設定
# ============================================================================
# 速度(mm/s)を算出するための調整値（路面から30mmの位置にセンサー設置で0.1程度）
