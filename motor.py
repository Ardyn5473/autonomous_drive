...
try:
    ...
    class MotorROSNode(Node):
        def __init__(self, motor):
            ...
            self.motor = motor  # Motorクラスのインスタンスを受け取る
        ...
        def steering_callback(self, msg):
            ...
            self.get_logger().info(f"Steering set to: {msg.data}")

        def throttle_callback(self, msg):
            self.motor.set_throttle_pwm_value(msg.data)
            self.get_logger().info(f"Throttle set to: {msg.data}")
        ...
        def spin(self):
            try:
                ...
                print("ROS2 node stopped by user.")
            finally:
                if rclpy.ok():
                    self.destroy_node()
                    rclpy.shutdown()
